import { signVapidJwt, encryptPayload } from '../push-helper-internal.js';
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!VAPID_PRIVATE || !VAPID_PUBLIC) return Response.json({ error: 'VAPID keys missing from env' }, { status: 500 });
    ...
    if (!Array.isArray(rows) || !rows.length) {
      return Response.json({ error: 'No subscription found in database. Allow notifications first.' }, { status: 404 });
    }

    const results = [];
    const payload = JSON.stringify({ title: 'ZenLootX — Test', body: 'Push notifications are working!', url: '/notifications' });

    for (const row of rows) {
      const sub = typeof row.subscription === 'string' ? JSON.parse(row.subscription) : row.subscription;
      if (!sub?.endpoint) { results.push({ error: 'no endpoint in subscription' }); continue; }
      ...
      try {
        ...
        let extraHeaders = {};
        if (hasKeys) {
          pushBody = await encrypt(sub.keys, payload, VAPID_PRIVATE, VAPID_PUBLIC);
          extraHeaders = { 'Content-Type': 'application/octet-stream', 'Content-Encoding': 'aes128gcm' };
        }
        const fcmRes = await fetch(sub.endpoint, {
          method: 'POST',
...
// Inline VAPID sign so we don't need a separate export from push-helper
function b64u(buf) {
...
async function signVapid(endpoint, privB64, pubB64) {
  ...
  const key = await crypto.subtle.importKey('jwk', {
    kty: 'EC', crv: 'P-256',
    x: b64u(pub.slice(1, 33)), y: b64u(pub.slice(33)), d: privB64,
...
async function encrypt(keys, plaintext) {
  ...
  const prk = await hkdfX(u8(auth), sharedSecret);
