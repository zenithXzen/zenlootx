...
**Footer socials + mobile 2x2 (via /impeccable, NOT yet pushed):** Added "Follow us" column with Facebook (https://www.facebook.com/share/1C7g35mnH1/) + Discord (https://discord.gg/zWKgscXVzb) icon buttons — 44px, open in new tab, accent hover. Desktop footer now 5 cols. Mobile (≤900 & ≤640) is a 2x2: brand spans full width on top, then Marketplace|Support, Company|Follow us — fixes the "one long footer" complaint, distributes weight both sides. Verified desktop + 390px, no overflow.

**Footer redesigned (via /impeccable, earlier — pushed as 95839c0):** Fixed WCAG contrast fails — column headers `h4`, copyright, and legal line moved off `text-faint` (~4.16:1 fail) to `text-dim` (now 7.87:1). Removed the fake "All systems operational" status indicator (integrity: no real monitoring behind it). Elevated DTI registration into a real pill badge in the brand column — "DTI-registered · No. 8247950" with check icon, links to `/FAQ/DTI.pdf` (7.29:1). Footer link hover now → accent. Bottom bar simplified: copyright + "Operated by ZENLOOTX ONLINE SHOP". Removed unused `.status-online`/`.status-dot` CSS. Verified desktop + 390px mobile, no overflow.
