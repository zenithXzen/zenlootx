...
async function initNav(user) {
  ...
  el.innerHTML = `
    ...
    <a href="/notifications" id="navBellBtn" aria-label="Notifications" title="Notifications" style="position:relative;display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:8px;background:var(--bg-elevated);border:1px solid var(--border-hi);color:var(--text-dim);flex-shrink:0;transition:border-color 0.18s,color 0.18s;">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0"/></svg>
    </a>
  ...
  bellBtn.addEventListener('mouseenter', () => { bellBtn.style.borderColor = 'var(--accent)'; bellBtn.style.color = 'var(--text)'; });
  bellBtn.addEventListener('mouseleave', () => { bellBtn.style.borderColor = 'var(--border-hi)'; bellBtn.style.color = 'var(--text-dim)'; });
...
// ─── Relative time label (used by disputes, notifications, seller-dashboard) ──
...
// ─── Tier badge icon (used by account, profile, public-profile) ──
function getTierIcon(tier, color) {
  const c = color.startsWith('url') ? 'url(#dg)' : color;
  const grad = color.startsWith('url') ? `<defs><linearGradient id="dg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#a78bfa"/><stop offset="33%" stop-color="#38bdf8"/><stop offset="66%" stop-color="#34d399"/><stop offset="100%" stop-color="#fbbf24"/></linearGradient></defs>` : '';
  const f = `fill="${c}"`;
  switch(tier) {
    case 1: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M12 2L4 5.5V11c0 5.2 3.6 9.8 8 11 4.4-1.2 8-5.8 8-11V5.5L12 2z"/></svg>`;
    case 2: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M9 2h6l1.5 7h-9L9 2z"/><circle ${f} cx="12" cy="16" r="6"/></svg>`;
    case 3: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M8 3h8v7a4 4 0 01-8 0V3z"/><path ${f} d="M4 5h4v4H5a2 2 0 010-4zM20 5h-4v4h3a2 2 0 000-4z"/><rect ${f} x="10" y="14" width="4" height="3"/><rect ${f} x="7" y="17" width="10" height="2" rx="1"/></svg>`;
    case 4: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M9 2h6v8a3 3 0 01-6 0V2z"/><path ${f} d="M3 4h6v5H4.5A2.5 2.5 0 013 4zM21 4h-6v5h4.5A2.5 2.5 0 0021 4z"/><rect ${f} x="11" y="13" width="2" height="3"/><rect ${f} x="7" y="16" width="10" height="2" rx="1"/></svg>`;
    case 5: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M12 2l1.2 3.5H17l-3 2.2 1.1 3.3L12 9l-3.1 2 1.1-3.3L7 5.5h3.8z"/><path ${f} d="M9 10h6v5a3 3 0 01-6 0V10z"/><rect ${f} x="11" y="17" width="2" height="2"/><rect ${f} x="7" y="19" width="10" height="2" rx="1"/></svg>`;
    case 6: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M7 7l2-5 3 4 3-4 2 5H7z"/><path ${f} d="M7 7h10v6a5 5 0 01-10 0V7z"/><rect ${f} x="10" y="15" width="4" height="2"/><rect ${f} x="7" y="17" width="10" height="2" rx="1"/></svg>`;
    case 7: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M7.5 3h9l4 5.5H3.5L7.5 3z"/><path ${f} d="M3.5 8.5L12 21l8.5-12.5H3.5z"/></svg>`;
    case 8: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M3 19V8l3.5 7L10 5l2 8 2-8 3.5 10L23 8v11H3z"/></svg>`;
    case 9: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M2 20V9l4 7.5L9.5 4 12 13l2.5-9L18 16.5 22 9v11H2z"/><circle ${f} cx="6" cy="9" r="1.8"/><circle ${f} cx="12" cy="4.5" r="1.8"/><circle ${f} cx="18" cy="9" r="1.8"/></svg>`;
    case 10: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M12 1.5l1.5 4H18l-4 3 1.5 4.5L12 10.5 8.5 13 10 8.5 6 5.5h4.5z"/><path ${f} d="M9.5 12h5v4a2.5 2.5 0 01-5 0V12z"/><path ${f} d="M4 14h5.5v3.5H6A2 2 0 014 14zM20 14h-5.5v3.5H18a2 2 0 002-2V14z"/><rect ${f} x="10" y="18" width="4" height="1.5"/><rect ${f} x="7" y="19.5" width="10" height="2" rx="1"/></svg>`;
    default: return `<svg viewBox="0 0 24 24" fill="none">${grad}<path ${f} d="M7 3h10v2H7zM5 5h14l-1 8H6zm1 9h12l-2 3H8zm2 3h6l-3 2z"/></svg>`;
  }
}

// ─── Banned-account check (used by account, profile, create-listing) ──
async function isBanned(userId) {
  const { data } = await sb.from('profiles').select('is_banned').eq('id', userId).single();
  if (data?.is_banned) { await sb.auth.signOut(); window.location.href = '/banned'; return true; }
  return false;
}
...
async function initNotifBadge(userId) {
  function setBadge(count) {
    ...
    document.querySelectorAll('a[href="/notifications"]').forEach(link => {
      ...
      badge.style.cssText = link.id === 'navBellBtn'
        // Pinned to the bell's top-right corner
        ? 'position:absolute;top:-6px;right:-6px;display:inline-flex;align-items:center;justify-content:center;min-width:17px;height:17px;border-radius:999px;background:var(--accent);color:var(--bg-base);font-size:10px;font-weight:700;padding:0 4px;line-height:1;border:2px solid var(--bg-base);'
